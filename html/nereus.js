...
var dArr = ["time.mo","time.tu","time.we","time.th","time.fr","time.jun","time.sa","time.su"];
var mArr = ["time.jan","time.feb","time.mar","time.apr","time.may","time.jun","time.jul","time.aug","time.sep","time.oct","time.nov","time.dec"];
var wArr = ["time.first","time.second","time.third","time.fourth","time.last"];
var dtF = ["DD.MM.YY","DD/MM/YY","DD-MM-YY","YY/MM/DD","YY-MM-DD"];
var tmF = ["HH:MI:SS P","HH.MI:SS P","HH24:MI:SS","HH24.MI:SS","HH:MI P","HH.MI P","HH24:MI","HH24.MI"];
...
$('div#menu a.pure-menu-link').click(function(event) {
	...
	if (lastmenu == 'home') if (isRealValue(tmt)) window.clearTimeout(tmt);
...
function psb() {
	   ...
	   sb('#dstEndDay', dArr);
	   sb('#mm',mArr);
	   sb('#dstStartMonth',mArr);
	   sb('#dstEndMonth',mArr);
	   sb('#dstStartWeek',wArr);
	   sb('#dstEndWeek',wArr);
	   sb('#dtF',dtF);
	   sb('#tmF',tmF);
}
...
function inf() {
	$.ajax({ url:'getinfo.cgi',
	    ...
	    contentType:"application/json; charset=utf-8",
		success:function(data) {
				if (!manual) timersled = window.setTimeout(inf, 30000);
...
function st() {	
	...
	$("#showtime").text(res);
	if (unixtime++ == (uts+60)) {
		if (isRealValue(tmt)) window.clearTimeout(tmt);
	    tmt=window.setTimeout('dct()',1000);
	} else {
		tmt=window.setTimeout('st()',1000);
	}
}
